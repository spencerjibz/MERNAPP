...
class App extends Component {
      ...
      event === "uploadphoto"
        ? axios
            .post("/api/photo", data, {
              headers: {
                "content-type":
                  "multipart/form-data; boundary=----WebKitFormBoundaryAtnbRG4IWrG5y81e",
              },
            })
            .then((v) => {
              v.data.hasOwnProperty("error")
                ? this.setState({ isError: true, Errmessage: v.data.error })
                : this.setState({
                    Nophoto: false,
                    photodata: [
                      `/api/profile-photo/${this.props.user.username}`,
                    ],
                  });
            })
        : axios
            .post("/api/updatephoto", data, {
              headers: {
                "content-type":
                  "multipart/form-data; boundary=----WebKitFormBoundaryAtnbRG4IWrG5y81e",
              },
  ...
  handleFiles(e) {
    ...
    if (fil !== undefined && e.target.files.length > 0) {
      e.target.id.includes("updatephoto")
        ? this.setState({ file: { event: "updatephoto", file_type: fil } })
  ...
  fetchPhoto() {
    axios
      .post("/api/profile-photo", { email: this.props.user.username })
      .then((v) => {
        v.data.hasOwnProperty("error")
          ...
          : this.setState({
              Nophoto: false,
              isError: false,
              photodata: [`/api/profile-photo/${this.props.user.username}`],
            });
      ...
      <div className="container" id="container">
        ...
        <div className="jumbotron" id="grid">
          {this.state.Nophoto === false
            ? this.state.photodata.map((result, i) => (
                <Photo key={i} data={result} />
              ))
            : null}
          ...
          {this.state.isUser === true
            ? this.state.userdata.map((results, i) => (
                <Results key={i} user={results} />
              ))
            : null}
          <br />
    ...
    return (
      <div id="Results" className="text-right">
        <ul className="form-group">
          <li name="id">
            {" "}
            ID:{this.props.user !== undefined ? this.props.user._id : null}
          </li>

          <li name="name">
            {" "}
            Name: {this.props.user !== undefined ? this.props.user.name : null}
          </li>
    ...
    return (
      <div>
        {this.props.photoStatus === true ? (
          <button className="btn-default" id="but">
            {" "}
            uploadimage
          </button>
        ) : null}
        <div className="container" id="forms">
          {this.props.photoStatus === true ? (
            <form
              ...
              method="post"
              id="myform"
              className="form-upload form-group"
