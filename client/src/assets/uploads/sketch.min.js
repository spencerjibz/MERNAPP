...
function setup() {
  (canv = createCanvas(400, 400).position(700, 100)),
    ...
    slid.hide(),
    (container = select(".jumbotron")),
    (filename = createP("kk").hide().position(100, 150)),
    container.child(filename),
    container.child(vol),
...
function handleFile(a) {
  switch (
    (print(a),
    filename.html(a.name).addClass("alert-info").show(),
    fileup.hide(),
    a.type)
  ) {
    case "image":
...
function webcam() {
  img ||
    vid ||
    aud ||
    ((cap = createCapture(VIDEO)), container.child(cap), cap.hide());
}
function action() {
