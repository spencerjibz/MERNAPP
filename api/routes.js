...
jwt.verify(req.token,config.TOKEN_SEC,(err,authdata)=>{
    if(err){
     ...
     err? res.json({code:404,error:'Not Found'}):
     delete user._doc.password
     delete user._doc.__v
     res.json({user})
...
check('email', 'Email must between 10 and 100 characters long').isLength({min:10,max:100}),

check('password', 'password is required').not().isEmpty(),
check('password', 'passwords must be atleast 8 characters long and contain at least one number,one lowercase and an Uppercase letter ').isLength({
    min: 8
}).matches(/\d/).matches(/[a-z]/).matches(/[A-Z]/),

check('confpassword', 'Passwords must match').exists().custom((value,{req})=>{return value===req.body.password})
    ...
    if (!errors.isEmpty()) {
        return res.json({ code:422,
            ValidationErrors: getUnique(errors.array(),'param').map(v=> {delete v.value;return v})
        });
    }
else {
      ...
      if(test){
        ...
        User.createUser(newUser,function(err,user){
            if(err){
            res.json({code:404,error:`failed to create new user because the user already exists`})
            }
            else {
...
router.post('/deleteone',authCheck,(req,res)=>{
  jwt.verify(req.token,config.TOKEN_SEC,(err,data)=>{
   err? res.json({code:404,error:err.message}):req.body.email !== data.user.username?res.json({code:404,error:`invalid username (email address)`}):
...
let test = Object.keys(req.body).length > 0 && req.body !== null&&req.body.hasOwnProperty('email')
 !test?res.json({code:401,error:`no email address provided, can't complete`}):
         ...
         if (!user) {
             ...
             transporter.sendMail(mailOptions, function (error, info) {
                 if (error) {
                     ...
                     res.json({error: 'Opps !! connection failed, try again later'})
                     
                 } else {
                     res.json({message: `Reset instructions  have been sent to your email, check your inbox in a few minutes`})
   ...
   if (!errors.isEmpty()) {
    ...
    let  mailOptions = {
     ...
     transporter.sendMail(mailOptions,(err,info)=>{
         if(err){
             res.json({code:'404',error:`Opps, connection failed try again later`})
         }
         else{
             res.json({message:`Hello ${req.body.name}  your message was successful recieved, we will get back to  me as soon as possible`})
         }
     })    
      ...
      if (err) {
        log(err)
         ...
         res.json({message:'image uploaded successfully',img})
          log(`${userData.user.name} has added a new photoimage at ${req.file.path}`)
      }
  })
 ...
 if( err || req.file === undefined ||req.file.path===undefined||req.file.path===null) { res.json({
    ...
    Photo.Updatephoto(userdata.user.username, photopath, (err, photodata) => {
        ...
        if (err || photodata === null) {
            res.json({
                code: 404,
                error: err !== undefined && err !== null ? err.message : 'no photo info found'
            })
        } else {
