...
module.exports.Updatephoto = function (username, imagepath, cb) {
    photoapi.findOne({
        username: username
        ...
        if (err || ismatch == null) {
            ...
            IsLinux === true ?
                mypath = path.resolve(`.\client\build\images\ ${ismatch.photoPath.replace(/\s/g,'')}`): mypath = path.resolve(`./client/build/images${ismatch.photoPath.replace(/\s/g,'')}`)
            fs.unlink(path.normalize(mypath), function (err) {
  ...
  if (err) {
      cb(err)
                    ...
                    photoapi.findOneAndUpdate({
                        ...
                        photoPath: imagepath.replace(/client\\+build\\+images\\/g, '')
                    }, cb)
...
module.exports.Deletephoto = function (username, cb) {
    ...
    photoapi.Findphoto(username, function (err, photo) {
        if (photo !== null) {
            ...
            IsLinux === true ? imagepath= path.resolve(`.\client\build\images\ ${photo.photoPath.replace(/\s/g,'')}`.replace(/\s/g,'')):
                imagepath = path.resolve(`./client/build/images/${photo.photoPath.replace(/\s/g,'')}`)
            //  find out if the file exists in the file system before deleting it.  function above checks for the desired file
            fs.access(path.normalize(imagepath), function (err) {
                if (err) {
                    log(err)
                    ...
                    fs.unlink(path.normalize(imagepath), function (err, done) {
                        if (err) {
                            log(err)
                        } else {
                            photoapi.findOneAndDelete({
