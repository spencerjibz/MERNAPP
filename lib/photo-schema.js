...
module.exports.Updatephoto = function (username, imagepath, cb) {
    photoapi.findOne({
        ...
        if (err || ismatch == null) {
            ...
            IsLinux === true ?
                mypath = `.\client\src\assets\ uploads\ ${ismatch.photoPath.replace(/\s/g,'')}`: mypath = `./client/src/assets/uploads/${ismatch.photoPath.replace(/\s/g,'')}`
  ...
  if (err) {
      cb(err)
                    ...
                    photoapi.findOneAndUpdate({
                        ...
                        photoPath: imagepath.replace(/client\\+src\\+assets\\uploads\\/g, '')
                    }, cb)
...
module.exports.Deletephoto = function (username, cb) {
    ...
    photoapi.Findphoto(username, function (err, photo) {
        if (photo !== null) {
            ...
            IsLinux === true ? imagepath= `.\client\src\assets\ uploads\ ${photo.photoPath.replace(/\s/g,'')}`.replace(/\s/g,'') :
                imagepath = `./client/src/assets/uploads/${photo.photoPath.replace(/\s/g,'')}`
